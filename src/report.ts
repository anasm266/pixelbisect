...
import type { CommitInfo, ComparisonResult, ComputedStyleDifference, EvaluationRecord, ResolvedConfig } from './types.js';
...
function configRows(config: ResolvedConfig): string {
  const values: Array<[string, unknown]> = [
    ['Repository', path.basename(config.repoPath)], ['Good ref', config.goodCommit], ['Bad ref', config.badCommit],
...
function styleRows(differences: ComputedStyleDifference[]): string {
  return differences.map((difference) => `<tr>
    <td><code>${escapeHtml(difference.property)}</code></td>
    <td>${escapeHtml(difference.lastGood || '—')}</td>
    <td>${escapeHtml(difference.firstBad || '—')}</td>
  </tr>`).join('');
}

function diffHtml(diffText: string): string {
  return diffText.split('\n').map((line) => {
    let className = 'diff-context';
    if (line.startsWith('diff --git ') || line.startsWith('index ') || line.startsWith('--- ') || line.startsWith('+++ ')) className = 'diff-meta';
...
export async function generateReport(input: GenerateReportInput): Promise<string> {
  ...
  const styleDifferences = input.styleDifferences ?? [];
...
<link rel="icon" href="data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 64 64'%3E%3Crect width='64' height='64' rx='14' fill='%23323437'/%3E%3Cpath d='M16 16h14v14H16zm18 0h14v14H34zM16 34h14v14H16z' fill='%23e2b714'/%3E%3Cpath d='M34 34h14v14H34z' fill='%23f2f2f0'/%3E%3C/svg%3E">
<style>
...
.hero{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:48px;align-items:end;padding:54px 2px 34px}.hero-copy{min-width:0}.eyebrow{display:flex;align-items:center;gap:9px;color:var(--accent-ink);font-size:11px;font-weight:700;letter-spacing:.11em;text-transform:uppercase}.eyebrow:before{content:"";width:7px;height:7px;border-radius:50%;background:var(--accent)}.hero h1{font-size:clamp(32px,5vw,54px);line-height:1.08;letter-spacing:-.055em;margin:12px 0 0;font-weight:650;max-width:820px}.hero h1 code{color:var(--accent-ink);font:inherit}.hero p{color:var(--muted);margin:13px 0 0;font-size:15px;overflow-wrap:anywhere}.hero-actions{display:flex;flex-wrap:wrap;gap:8px;margin-top:20px}.action{appearance:none;border:1px solid var(--line);border-radius:6px;padding:8px 11px;color:var(--ink);background:var(--surface);font:600 11px/1 ui-monospace,SFMono-Regular,Menlo,Monaco,Consolas,monospace;cursor:pointer}.action:hover{border-color:var(--accent);background:var(--accent-soft)}.action:focus-visible{outline:3px solid var(--accent);outline-offset:2px}.action.primary{border-color:#d7ad10;background:var(--accent);color:#292b2e}.summary{display:grid;grid-template-columns:repeat(3,minmax(92px,1fr));gap:26px;margin:0}.summary div{min-width:0}.summary dt{color:var(--subtle);font-size:10px;letter-spacing:.08em;text-transform:uppercase}.summary dd{margin:2px 0 0;color:var(--ink);font-size:20px;line-height:1.25;font-weight:700;letter-spacing:-.04em;font-variant-numeric:tabular-nums}
...
@media print{body{background:#fff;border:0}.shell{width:100%;padding:0}.card{box-shadow:none;break-inside:avoid}.slider,.divider,.hero-actions{display:none}.after-clip{clip-path:none}.hero h1 code{color:#6d5800}.topbar{padding-top:0}}
</style>
</head>
<body>
<main class="shell">
  <div class="topbar">
    ...
    <span class="generated-at">generated <time datetime="${escapeHtml(generated.toISOString())}">${escapeHtml(displayDate(generated))}</time></span>
  </div>
  <header class="hero">
    <div class="hero-copy">
      ...
      <div class="hero-actions">
        <button class="action primary" id="copy-hash" type="button" data-value="${escapeHtml(c.hash)}">Copy culprit hash</button>
        <button class="action" id="print-report" type="button">Print / save PDF</button>
      </div>
    </div>
    <dl class="summary">
  ...
  ${styleDifferences.length > 0 ? `<section class="card" aria-labelledby="styles-title"><div class="card-head"><h2 id="styles-title">Rendered style changes</h2><span>${styleDifferences.length} changed propert${styleDifferences.length === 1 ? 'y' : 'ies'}</span></div><div class="table-wrap"><table class="style-table"><thead><tr><th>Property</th><th>Last good</th><th>First bad</th></tr></thead><tbody>${styleRows(styleDifferences)}</tbody></table></div></section>` : ''}
  ...
  <section class="card"><div class="card-head"><h2>Culprit Git diff</h2><span class="diff-title">${escapeHtml(g.shortHash)} → ${escapeHtml(c.shortHash)}</span></div><pre aria-label="Git diff">${diffHtml(input.diffText)}</pre></section>
...
(() => {
  ...
  copyButton.addEventListener('click', async () => {
    ...
    setTimeout(() => { copyButton.textContent = 'Copy culprit hash'; }, 1400);
  });
